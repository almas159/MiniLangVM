...
Осторожно: если преподаватель строго смотрит формулировку A.1.7, там может требоваться не только вывод типа переменных, но и вывод типа возвращаемого значения функций. У меня уверенно реализован именно вывод типа переменных через `var`.
...
Уровень: 1
Баллы: 5
Статус: частично/скорее можно заявлять, но осторожно
...
Осторожно: A.1.9 в ТЗ может требовать ещё все составные присваивания:
...
Если у меня есть только `+=`, `-=`, `++`, `--` и битовые операции, то лучше на защите говорить:

```
У меня реализована основная часть A.1.9: битовые операции, сдвиги, runtime-проверка сдвигов, инкремент/декремент и часть compound assignment.
```
...
Уровень: 1
Баллы: 5
Статус: можно уверенно заявлять
...
```
Это доп A.1.13. Я реализовал sizeof, typeid и typeof. Они работают на этапе анализа типов и позволяют посмотреть информацию о типе выражения.
```
...
Если вложенность реально не поддержана, этот доп лучше не считать как полноценные 5 баллов.

На защите можно сказать осторожно:

```
У меня есть блочные комментарии. Если требуется именно вложенность, это ограничение текущей реализации.
```
...
Ещё важно: моя реализация ближе к type erasure, а не к полноценной monomorphization.

То есть semantic analyzer проверяет типы, но отдельные копии функции под каждый тип не создаются.

На защите лучше говорить честно:

```
В проекте реализованы generic-функции с явными type arguments. Generic structs не реализованы, поэтому A.2.16 у меня закрыт не полностью по максимальной формулировке.
```
...
Уровень: 3
Баллы: 20
Статус: можно заявлять, если учитель принимает generic-функции как базу
...
```
Это доп A.3.7. При вызове generic-функции semantic analyzer выводит type arguments из типов фактических аргументов.
```

Осторожно: этот доп логически зависит от A.2.16. Если преподаватель не засчитает A.2.16 из-за отсутствия generic structs, то A.3.7 тоже может стать спорным. Но как отдельная фича для generic-функций она работает.
...
Уровень: 2
Баллы: 10
Статус: можно заявлять, если overload tests проходят
...
```
Это доп A.2.8. Перегрузка решается на этапе semantic analysis. В AST сохраняется конкретный functionIndex выбранной функции.
```
...
Уровень: 3
Баллы: 20
Статус: можно заявлять, если A.2.8 работает
...
У меня поддерживаются приведения:
...
```
Это доп A.3.1. Сначала ищется точное совпадение перегрузки. Если его нет, semantic analyzer пробует безопасные неявные приведения. Если подходящий вариант один, он выбирается. Если вариантов несколько, это ambiguity error.
```
...
```
Это доп B.2.5. Вместо прямой интерпретации AST я генерирую собственный байткод и исполняю его на отдельной стековой VM.
```
...
```
Это доп B.1.8. CLI позволяет посмотреть внутренние этапы работы компилятора и отдельно сгенерировать ASM.
```
...
Уровень: 1
Баллы: 5
Статус: можно заявлять, если runner лежит в scripts и проходит
...
```
Это доп B.1.10. Я сделал golden test runner, который сравнивает stdout, stderr и exit code с эталонными файлами.
```
...
Уровень: 3
Баллы: 20
Статус: указывать только если реально работает
...
Если `--use-stdlib` ещё выдаёт:

```
unknown option '--use-stdlib'
```

то A.3.15 пока нельзя заявлять как готовый доп.
...
На защите можно говорить только после проверки:

```
Это доп A.3.15. Библиотека реализована на языке MiniLangVM и подключается через --use-stdlib.
```

---

# Возможности, которые есть в проекте, но я не считаю отдельными допами

Некоторые вещи есть в языке, но лучше не считать их отдельными баллами, если в ТЗ нет отдельного пункта или если они являются частью базовой реализации.
...
Это важная часть проекта, но я бы не писал это как отдельный доп, если нет точного пункта в ТЗ.
...
Это хорошая фича лексера, но если нет отдельного номера в ТЗ, лучше считать её частью общей реализации языка.
...
Это тоже полезно для демонстрации, но отдельно как доп я бы не считал без точного пункта.
...
Но структуры, скорее всего, относятся к базовой части языка или к общей системе типов. Отдельным доп. пунктом их лучше не считать, если в ТЗ нет специального задания.

## Обобщённые массивы и массивы внутри структур

Это сильная часть проекта, но номер допа зависит от формулировки ТЗ.

Если в ТЗ есть отдельный пункт про обобщённые контейнеры/массивы/составные типы, тогда можно привязать. Если нет, лучше писать как расширение системы типов, но не считать отдельно в баллы.
